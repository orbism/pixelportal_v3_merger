...
class MintPixelsDialogStore extends Reactionable(Navigable<MintModalView, Constructor>(EmptyClass)) {
  ...
  init() {
    ...
    this.react(
      () => [this.srcCurrency, this.pixelCount, AppStore.web3?.address],
      async () => {
        if (AppStore.web3?.address && Number(this.pixelCount) > 0) {
          this.setSrcCurrencyContract();
  ...
  reset() {
    ...
    this.approveInfiniteDOG = false;
    this.approveInfiniteVault = false;
    this.txHash = null;
    this.srcCurrency = "DOG";
    this.srcCurrencyContract = null;
    this.cowSimpleQuote = null;
    this.recentQuote = null;
    this.srcCurrencyBalance = { humanReadable: null, bigNumber: null };
    this.isLoading = false;
    this.isOrderComplete = false;
    this.cowSwapOrderID = undefined;
    this._pollOrdersTick = 0;
    this.oldPixels = [];
    this.diffPixels = [];
  ...
  setSrcCurrencyContract() {
    ...
    if (this.srcCurrency === "DOG20") {
      ...
      const contractAddress = env.app.availableTokens[this.srcCurrency]?.contractAddress;
      if (!contractAddress) {
        console.error("Contract address is not available for the currency:", this.srcCurrency);
        return;
      }
  ...
  async getSrcCurrencyBalance() {
    if (!this.srcCurrencyContract) {
      console.error("Source currency contract is not set.");
      return;
    }
