 ...
 * {ERC721Enumerable}.
 */
contract ERC721CustomUpgradeable is Initializable, ContextUpgradeable, ERC165Upgradeable, IERC721, IERC721Metadata {
    ...
    function balanceOf(address owner) public view virtual override returns (uint256) {
        if (owner == address(0)) revert ERC721BalanceQueryForZeroAddress();
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        if (!(_msgSender() == owner || isApprovedForAll(owner, _msgSender()))) {
            revert ERC721TransferCallerNotOwnerNorApproved();
        }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        if (!_isApprovedOrOwner(_msgSender(), tokenId)) revert ERC721TransferCallerNotOwnerNorApproved();
    ...
    function _safeTransfer(address from, address to, uint256 tokenId, bytes memory _data) internal virtual {
        _transfer(from, to, tokenId);
        if (!_checkOnERC721Received(from, to, tokenId, _data)) revert ERC721TransferToNonERC721ReceiverImplementer();
    }
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        if (to == address(0)) revert ERC721MintToZeroAddress();
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        if (ERC721CustomUpgradeable.ownerOf(tokenId) != from) revert ERC721TransferOfTokenNotOwn();
